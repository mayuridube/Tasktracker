...
<body>
    ...
    <script>
            ...
            generatedWebsites.forEach(website => {
                ...
                li.innerHTML = \`
                    ...
                    <div class="website-actions">
                        <a href="\${website.publicUrl}" target="_blank" class="website-link">View</a>
                        <button class="edit-button" data-id="\${website.id}">Edit</button>
                    </div>
                \`;
        ...
        // Open website editor
        function openWebsiteEditor(websiteId) {
            const currentWebsite = generatedWebsites.find(website => website.id === websiteId);
            if (!currentWebsite) {
                return;
            }
            
            // Show editor and hide generator panel
            document.getElementById('websiteEditor').style.display = 'block';
            document.getElementById('websiteGenerator').style.display = 'none';
            
            // Set editor title
            document.getElementById('editorTitle').textContent = 'Edit Website: ' + currentWebsite.description.substring(0, 30) + '...';
            
            // Store current website ID
            window.currentWebsiteId = websiteId;
            
            // Load files list
            loadWebsiteFiles(websiteId);
            
            // Set up preview frame
            refreshPreview();
            
            // Set up event listeners for editor components
            setupEditorEventListeners();
        }
        
        // Load website files
        async function loadWebsiteFiles(websiteId) {
            try {
                const response = await fetch('/api/websites/' + websiteId + '/files');
                if (response.ok) {
                ...
                addErrorToConsole('Failed to load website files');
            }
        }
        ...
        async function openFile(filename) {
            try {
                ...
                const response = await fetch('/api/websites/' + websiteId + '/files/' + filename);
                if (response.ok) {
                ...
                addErrorToConsole('Failed to open file: ' + filename);
            }
        }
        ...
        // Activate a file tab
        function activateFileTab(filename) {
        ...
        // Remove a file tab
        function removeFileTab(filename) {
            ...
            // If this was the active tab, open another one if available
            if (window.currentFilename === filename) {
                const remainingTabs = document.querySelectorAll('.file-tab');
                if (remainingTabs.length > 0) {
                    openFile(remainingTabs[0].dataset.filename);
                } else {
                    // Clear editor if no tabs left
                    const codeEditor = document.getElementById('codeEditor');
                    codeEditor.value = '';
        ...
        async function saveCurrentFile() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    addSuccessToConsole('Saved ' + filename);
                } else {
                    const error = await response.json();
                    addErrorToConsole('Failed to save file: ' + error.error);
                }
            } catch (error) {
                console.error('Error saving file:', error);
                addErrorToConsole('Network error while saving file');
            }
        }
        ...
        async function createNewFile(filename) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // Open the new file
                    openFile(filename);
                    addSuccessToConsole('Created new file: ' + filename);
                } else {
                    const error = await response.json();
                    addErrorToConsole(`Failed to create file: ${error.error}`);
                }
            } catch (error) {
                console.error('Error creating file:', error);
                addErrorToConsole('Network error while creating file');
            }
        }
        ...
        function refreshPreview() {
            ...
            previewFrame.src = `/view/${websiteId}?t=${Date.now()}`;
        }
        
        // Add message to console
        function addMessageToConsole(message, type = 'info') {
            const consoleOutput = document.getElementById('consoleOutput');
            const messageElem = document.createElement('div');
            messageElem.className = `console-message ${type}`;
            messageElem.innerHTML = `<span class="timestamp">[${new Date().toLocaleTimeString()}]</span> ${message}`;
            consoleOutput.appendChild(messageElem);
            consoleOutput.scrollTop = consoleOutput.scrollHeight;
        }
        
        // Add error message to console
        function addErrorToConsole(message) {
            addMessageToConsole(message, 'error');
        }
        
        // Add success message to console
        function addSuccessToConsole(message) {
            addMessageToConsole(message, 'success');
        }
        ...
        function setupEditorEventListeners() {
            ...
            document.getElementById('backButton').addEventListener('click', () => {
                ...
                document.getElementById('websiteGenerator').style.display = 'block';
                
                // Clear current website state
                window.currentWebsiteId = null;
                window.currentFilename = null;
            });
            ...
            document.getElementById('runButton').addEventListener('click', () => {
                ...
                if (window.currentFilename) {
                    saveCurrentFile().then(() => {
                        refreshPreview();
                        addMessageToConsole('Website updated and refreshed');
                    });
                } else {
                    refreshPreview();
                    addMessageToConsole('Preview refreshed');
                }
            });
